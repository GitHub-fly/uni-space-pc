		...
		<div class="hy-index-mid ba-col-9">
			<v-card class="mx-auto" max-width="100%" max-height="600px">
				<v-card-text>
					...
					<!-- 标题输入框 -->
					<form>
						<v-text-field v-model="usertitle" :error-messages="nameErrors" :counter="10" label="标题"></v-text-field>
					</form>

					<!-- 首页富文本编辑框 -->
					<v-card class="mx-auto " max-width="100%" max-height="400px">
						<div id="editor"></div>
					</v-card>
					<v-card-actions>
						<v-btn color="orange" @click="topublish()" text>
							确认发表
						</v-btn>
					</v-card-actions>
				</v-card-text>
